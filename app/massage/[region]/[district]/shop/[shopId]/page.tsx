...
// 🌟 빌드 시 정적 경로 사전 생성 (구 단위 샵 상세 페이지)
export async function generateStaticParams() {
  const paths: { region: string; district: string; shopId: string }[] = [];
...
// 🌟 60가지 타이틀 & 40가지 디스크립션 연동 메타데이터 생성
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  // {loc} 치환 (예: "서울 강남구 출장 소프트스웨디시 마사지·홈타이 | 방문 안마 케어")
