...
// 🌟 1단: '출장'과 '마사지'가 절대 붙지 않고 중간에 수식어가 들어간 핵심 키워드 풀 (30개)
...
// 🌟 2단: CTR을 극대화하는 롱테일 서브 소구 문구 풀 (구분자 '｜' 뒤에 위치, 20개)
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const seedString = `${locationPrefix}-${shopId}-expanded-dong-shop-seo`;
  ...
  // 💡 [지역 구 동] [출장 1단 키워드]｜[2단 소구 문구] 구조로 약 35~40자 구성 (도메인/샵 이름 배제)
  ...
  return {
    ...
    alternates: {
      canonical: `https://tagtherest.netlify.app/${region}/${encodeURIComponent(safeDecode(district))}/${encodeURIComponent(safeDecode(dong))}/shop/${shopId}`,
    },
    openGraph: {
      title: formattedTitle,
      description: formattedDesc,
      url: `https://tagtherest.netlify.app/${region}/${encodeURIComponent(safeDecode(district))}/${encodeURIComponent(safeDecode(dong))}/shop/${shopId}`,
      locale: "ko_KR",
      type: "article",
    },
  };
    ...
    <div className="bg-[#050505] text-gray-100 min-h-screen flex flex-col font-sans selection:bg-amber-500 selection:text-black pb-24">
      ...
      <div className="fixed bottom-0 left-0 right-0 z-50 bg-[#08080a]/95 backdrop-blur-xl border-t border-amber-500/30 p-3 md:p-4 shadow-[0_-10px_25px_rgba(0,0,0,0.8)]">
        <div className="max-w-4xl mx-auto grid grid-cols-2 gap-3">
          ...
          <a href={`sms:${shop.phone}?body=${encodeURIComponent(`[${locationPrefix}]${shop.name} 예약 문의드립니다.`)}`} className="flex items-center justify-center gap-2 bg-neutral-900 text-white font-black py-3.5 rounded-2xl text-xs md:text-sm border border-white/10 active:scale-95 transition-transform">
            💬 간편 문자상담
          </a>
        </div>
      </div>
