...
// 🌟 2. '마사지'가 반드시 포함된 서비스 종류 100개 풀 생성기
function getServiceTypesPool(): string[] {
  const coreTechniques = ["스웨디시", "아로마", "타이", "스포츠", "힐링", "바디케어", "릴렉싱", "웰니스", "전문", "프리미엄"];
  const styles = ["감성 마사지", "맞춤형 코스 마사지", "전신 관리 마사지", "전문 테크닉 마사지", "집중 이완 마사지", "릴렉스 마사지 과정", "힐링 마사지 프로그램", "프리미엄 바디 마사지", "맞춤형 마사지 솔루션", "토탈 마사지 프로그램"];
...
// 🌟 3. '마사지'가 반드시 포함된 상세 설명 60개 풀 생성기
...
export async function generateMetadata({ params, searchParams }: PageProps): Promise<Metadata> {
  ...
  const seedString = locationKeyword + districtName + dongName + "tagtherest-massage-mashup";
  ...
  const finalTitle = `${locationKeyword} ${selectedModifier} ${selectedService} 안내 · 태그더레스트`;
